# Roadmap

## v0.1

- Canonical MCP visual envelopes.
- Wave barcode SVG.
- Sphere field SVG.
- Metadata scanner.
- Frame reassembly and hash validation.

## v0.2

- Animated optical frames for time-sliced transport.
- Browser sender and receiver pages.
- Camera input prototype with Canvas sampling.
- Reed-Solomon or fountain-code style redundancy.
- Perspective correction and stronger adaptive thresholding.
- Majority-vote repeated camera captures in the browser receiver.
- Forward error correction beyond CRC detection.

## v0.3

- MCP server adapter that only accepts decoded, validated, policy-authorized visual messages.
- Replay protection ledger.
- Bidirectional visual return frames.
- Latency and error-rate test harness.
